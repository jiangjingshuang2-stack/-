...
\begin{itemize}[leftmargin=2em]
    ...
    \item \texttt{sklearn Lasso} 对照结果表
\end{itemize}
...
\subsection{已整理好的图片文件夹}
为了方便组员直接写报告和制作 PPT，我们已经把建议优先使用的图片从 \texttt{results/} 中\textbf{复制}到单独文件夹 \texttt{report\_figures/}，并改成了更直观的文件名。该文件夹中的图片不会影响原始实验结果，主要作用是降低组员整理素材时的沟通成本。

当前已整理的代表性图片包括：
\begin{itemize}[leftmargin=2em]
    \item \texttt{01\_synthetic\_main\_convergence\_curve.png}
    \item \texttt{02\_synthetic\_main\_sparse\_recovery\_coefficients.png}
    \item \texttt{03\_repeat\_experiment\_mean\_iterations.png}
    \item \texttt{04\_scale\_comparison\_iterations.png}
    \item \texttt{05\_regularization\_path\_admm.png}
    \item \texttt{06\_phase\_transition\_exact\_success\_rate.png}
    \item \texttt{07\_phase\_transition\_relaxed\_success\_rate.png}
    \item \texttt{08\_diabetes\_convergence\_curve.png}
    \item \texttt{09\_wine\_red\_convergence\_curve.png}
    \item \texttt{10\_communities\_crime\_convergence\_curve.png}
\end{itemize}

\subsection{建议正文优先使用的图片}
如果正式报告正文篇幅有限，建议优先选择以下图片：
\begin{enumerate}[label=\arabic*.]
    \item \texttt{02\_synthetic\_main\_sparse\_recovery\_coefficients.png}
    \item \texttt{01\_synthetic\_main\_convergence\_curve.png}
    \item \texttt{05\_regularization\_path\_admm.png}
...
真实数据部分通常不需要把三张收敛曲线都放进正文，更推荐把其结论整理成一张汇总表；如果答辩时需要补图，可优先使用 \texttt{10\_communities\_crime\_convergence\_curve.png} 作为复杂真实数据代表图。
